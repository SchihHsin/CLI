# AI Era Dev Tools Design — AGENTS.md

> 此文件由 Codex 每次会话自动加载，取代旧的 context.md + process.log（两者已废弃）。
...
1. **`git push`** — 推送到远程仓库
2. **更新 AGENTS.md** — 将本次对话中的重要进展、决策、新约定写入本文件

不要等到会话结束才统一处理，每完成一个用户请求就执行。

## 最新交付（2026-08-21）

- 新增 `desktop-pet-design-ppt/`：独立的 6 页网页 PPT，主题为「方块独角兽」桌宠设计方案。
- 入口为 `desktop-pet-design-ppt/index.html`，包含 scroll-snap 翻页、总览与全屏交互；主视觉为同目录 `desktop-pet-concept.png`。
- 叙事主线：毛茸茸材质建立亲和力 → 借鉴《我的世界》的方块形态 → 蓝紫渐变形成安静陪伴感 → 桌面场景与打样验证。
...
| 6.5 | `s-m2d` | 语音模式路由 — 上下文决定落地方式 · Stage 02 · 2/5 | ~1618 |
| 6.6 | `s-m2e` | 情感感知 — 音调即上下文 · Stage 02 · 3/5 | ~1857 |
| 6.7 | `s-m2f` | Prompt 记忆补全 — 两字召回历史 · Stage 02 · 4/5 | ~2031 |
| 7 | `s-m2b` | 手势 Prompt — Flow 不中断 | 动态定位 |
| 8 | `s-m2a` | 光标即上下文 — 停顿触发 AI | 动态定位 |
| 9 | `s-m3b` | Goal 命令 — 开发者定义完成 | 动态定位 |
| 10 | `s-m1a` | Agent View — 多会话统一管理 | 动态定位 |
| 11 | `s-m1b` | 并行看板 — 所有 Agent 一屏总览 | 动态定位 |
| 11.5 | `s-m1c` | 实体状态灯 — AI Agent 进度跃出屏幕 | 动态定位 |
| 12 | `s-m5a` | 结果审核 — 多维可视化 | 动态定位 |
| 14 | `s-m5b` | **推理透视 — AI 停下来时看清内部**（超时按需展开，推理链三态，阻塞定位+干预） | 动态定位 |
| 15 | `s-m4b` | 团队记忆库 — 隐性知识共享 | 动态定位 |
| 16 | `s-m6a` | 光标伴侣 — 贴身 AI 操作引导 · Stage 02 · 5/5 | 动态定位 |
| 17 | `s-sum` | 总结 | 动态定位 |
...
### 组件概览
所有设计幻灯片（s-m1a / s-m1b / s-m2a / s-m2b / s-m2c / s-m2d / s-m2e / s-m2f / s-m3b / s-m4a / s-m4b / s-m5a / s-m5b / s-m6a，共14个）的 dp-right 区域 mockup-label 下方均有步骤指示条。
...
| *(2026-07-03)* | `/model` V2.0 共享额度池方案：新增基础模型池 / 高阶模型池两组展示，组头显示池级 5h / 7d 用量和重置时间；模型行只说明消耗哪个 pool，不重复显示独立 quota；高亮移动时底部 usage panel 跟随目标模型所属 pool 刷新；高阶池接近上限时组卡片、底部 panel、Sidebar pool 三处同步黄/红提醒。 |
| *(2026-07-03)* | `/model` 页面改为 V1/V2 tab 切换：顶部新增 `V1 · 单模型额度` 与 `V2 · 后续：分组共享额度` 两个 tab，默认展示 V1；点击 V2 后隐藏 V1，只展示后续分组共享额度方案。 |
| *(2026-07-03)* | 页面品牌从 OpenCode 改为 CodeArts TUI：HTML title、H1、平台标签、TUI 窗口标题栏均同步更新。 |

### hai-operator-design.html 专项工作

| commit | 内容 |
|---|---|
| *(2026-05-26)* | 首页 `s-cover` 按 `huashu-design` 风格重绘：仅改样式不改内容；引入演示稿式网格/规则线/大字锚点；新增自驱动蓝色点阵符号动画（`+/*/-/=/·` 变换）；右侧 HCI/HAI 双卡明确分为 **PPT 蓝色语义** 与 **终端紫色语义**，禁止混色 |
| `1eab472` | 新增 `hai-operator-design.html`：HAI × CANN 昇腾算子开发场景全新演示文稿（18个section） |
| `f7ce851` | 更新 AGENTS.md，记录第二份材料信息 |
...
| `188957e` | 新增 s-bg-signal：行业验证页（4卡：Codex Desktop Buddy / OpenAI×Ive / Devin / Copilot Voice） |
...
| *(2026-05-25)* | 第8页（s-m4a）记忆启动 dp-body 补充 AGENTS.md + context compact 真实痛点；BEFORE 面板改为"静态文件维护滞后 + compact 后遗忘当轮决策"两条具体场景 |
| *(2026-05-25)* | 第8页（s-m4a）mockup 重设计：从不清晰的卡片堆叠改为结构化终端加载序列——`hai start` 后三块记忆（会话上下文/性能基线/硬件约束矩阵）逐段出现，step 标签同步更新 |
| *(2026-05-25)* | 新增第16页 s-remote（移动监控）：Stage 05 · 随时介入。手机 mockup 展示 4 个 Agent 实时状态，步骤2收到 dtype-check format 不兼容推送，步骤3语音批准继续；引用 Codex Remote Control 2026 官方验证 |
...
| `91ab5d1` | 更新 AGENTS.md：记录 WCAG 修复、colors.css 共用色表、两处 UI 修复 |
| *(2026-05-25)* | **终端主题/PPT主题严格分离**：新增 `--ta #A78BFA`（紫色，终端专用强调色，WCAG AAA 7.7:1），将两个 HTML 文件中终端区域的所有 `color:var(--acc)` 改为 `color:var(--ta)`（hai-operator-design.html 56处，hai-cli-design.html 49处HTML+8处JS），同步更新 colors.css 和 AGENTS.md |
...
→ s-j-overview
→ s-m4a（记忆启动）
→ s-m2c（语音×光标）→ s-m2d（语音模式路由）→ s-m2e（情感感知）→ s-m2f（Prompt记忆补全）
→ s-m6a（光标伴侣）
→ s-m2b（手势调参）
→ s-m2a（光标上下文）
...
| 卡片 | 产品 | 背景色 | 印证 |
|---|---|---|---|
| ① | Codex Desktop Buddy（Anthropic 开源，ESP32-S3）| `#1a1a24` | s-m1c 实体状态灯 |
| ② | M5Stack Cardputer "Code w/ Codex" 现场活动 | `#eeecea` | AI 走向实体 |
| ③（wide）| Devin 2.0 多 Agent 并行看板 | `#0d0d14` | s-m1b 并行 Agent 看板 |

**Page 2（行业验证 2/2 — Agent 编排进化）：**
| 卡片 | 产品 | 背景色 | 印证 |
|---|---|---|---|
| ④ | Warp ADE 开源（Codex / Codex / Gemini CLI 共享上下文）| `#0d0d14` | 终端=Agent编排中心 |
| ⑤ | GitHub Copilot Agent Mode 跨文件自主编辑 | `#0d0d14` | s-m1a Agent 自主执行 |
| ⑥（wide）| Codex Agent View 多会话统一看板 | `#0a0a10` | s-m1a Agent View |
